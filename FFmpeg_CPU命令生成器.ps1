# =====================================================================================
# FFmpeg 批量转码命令生成器 (CPU 版)
# 版本: 1.1
# 环境: Windows 10 / PowerShell 5.1+
# 功能: 自动扫描视频文件,根据用户参数生成使用 CPU 编码的 FFmpeg 命令,并重建目录结构。
# =====================================================================================
...
function Get-UserConfiguration {
    Write-Host "--- FFmpeg 命令生成器 (CPU 版 V1.1) ---" -ForegroundColor Yellow
    ...
    # 2. 获取输出路径
    $outputPath = Read-Host -Prompt "请输入输出文件夹路径 (可选, 默认在源目录的同级创建 'encode_cpu' 文件夹)"

    # 3. 获取编码参数 (CPU)
    $encoder = Read-Host -Prompt "请输入视频编码器 [libx265 / libx264] (默认: libx265)"
    if ([string]::IsNullOrWhiteSpace($encoder)) { $encoder = "libx265" }

    $crf = Read-Host -Prompt "请输入视频质量 CRF 值 (数字越小质量越高) (默认: 23)"
    if ([string]::IsNullOrWhiteSpace($crf)) { $crf = "23" }
...
if ([string]::IsNullOrWhiteSpace($config.OutputPath)) {
    # 如果用户未提供输出路径, 则使用默认路径
    $outputPath = Join-Path -Path $defaultOutputPathBase -ChildPath "encode_cpu"
} else {
...
foreach ($file in $videoFiles) {
    ...
    # 构建FFmpeg命令字符串 (CPU版本)
    $command = "ffmpeg -i `"$($file.FullName)`" -c:v $($config.Encoder) -preset $($config.Preset) -crf $($config.CRF) -c:a copy `"$($outputFileFullName)`""
...
Write-Host "2. 使用 'FFmpeg 智能命令执行器' 来批量执行这些命令。"
Write-Host "提醒: CPU 编码速度通常慢于硬件编码，请耐心等待。"
Write-Host "========================================================================" -ForegroundColor Green
